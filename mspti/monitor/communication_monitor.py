...
from ..activity_data import CommunicationData
    ...
    def callback(self, origin_data: dict):
        try:
            if callable(self.user_cb):
                self.user_cb(CommunicationData(origin_data))
        except Exception as ex:
            print_error_msg(f"Call communication callback failed. Exception: {str(ex)}")
