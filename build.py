...
class BuildManager:
    ...
    def _archive_artifacts(self):
        ...
        for pattern in artifact_patterns:
            for artifact in output_dir.glob(pattern):
                if artifact.is_file():
                    logging.info("Archiving artifact: %s -> %s", artifact, artifacts_dir)
                    shutil.copy2(artifact, artifacts_dir)
    ...
    def run(self):
        ...
        if 'test' in self.args.command:
            ...
            env = os.environ.copy()
            env["BUILD_VERSION"] = self.args.version
            env["WHL_VERSION"] = self.args.version
            self._execute_command(["bash", "scripts/build.sh"], cwd=self.project_root, env=env)
