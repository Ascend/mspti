    ...
    ProfApiInject() noexcept
    {
        ...
        static const std::vector<std::pair<int, VOID_PTR>> CALLBACK_FUNC_LIST = {
            {PROFILE_REPORT_GET_HASH_ID_C_CALLBACK,
             Mspti::Common::ReinterpretConvert<VOID_PTR>(Mspti::Inject::Detail::MsptiGetHashIdImpl)},
            {PROFILE_REPORT_REG_TYPE_INFO_C_CALLBACK,
             Mspti::Common::ReinterpretConvert<VOID_PTR>(Mspti::Inject::Detail::MsptiRegReportTypeInfoImpl)},
        };
...
int32_t MsptiApiReporterCallbackImpl(uint32_t agingFlag, const MsprofApi* const data)
{
    ...
    switch (data->level)
    {
        case MSPROF_REPORT_RUNTIME_LEVEL:
            if (Mspti::Parser::CannApiParser::GetInstance().ReportApi(agingFlag, data) != MSPTI_SUCCESS)
            {
...
int32_t MsptiCompactInfoReporterCallbackImpl(uint32_t agingFlag, CONST_VOID_PTR data, uint32_t length)
{
    ...
    if (compact->level == MSPROF_REPORT_NODE_BASE_LEVEL && compact->type == MSPROF_REPORT_NODE_HCCL_OP_INFO_TYPE)
    {
        if (Mspti::Parser::CommunicationCalculator::GetInstance().AppendCompactInfo(agingFlag, compact) !=
            MSPTI_SUCCESS)
        {
            MSPTI_LOGE("Report Msprof Compact data to ParserManager failed.");
            return PROFAPI_ERROR;
...
int32_t MsprofDeviceStateImpl(VOID_PTR deviceState, uint32_t len)
{
    ...
    ProfSetDevPara* devPara = Mspti::Common::ReinterpretConvert<ProfSetDevPara*>(deviceState);
    MSPTI_LOGI("Receive device state callback, chipId: %u, devId: %u, isOpen: %d.", devPara->chipId, devPara->devId,
               devPara->isOpen);
    if (devPara->isOpen)
    {
