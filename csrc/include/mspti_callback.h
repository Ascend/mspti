...
#pragma GCC visibility push(default)
     ...
     */
    typedef void (*msptiCallbackFunc)(void *userdata, msptiCallbackDomain domain, msptiCallbackId cbid,
     ...
     *
     * @return MSPTI_SUCCESS on success
     * @return MSPTI_ERROR_NOT_INITIALIZED if MSPTI is not initialized
     * @return MSPTI_ERROR_WITHOUT_LD_PRELOAD if libmspti.so is not set in LD_PRELOAD environment variable
     * @return MSPTI_ERROR_INVALID_PARAMETER if @p subscriber, @p domain or @p cbid is invalid.
     */
     ...
     *
     * @note @b Thread-safety: a subscriber must serialize access to msptiGetCallbackState, msptiEnableCallback,
     * msptiEnableDomain, and msptiEnableAllDomains. For example, if msptiGetCallbackState(sub, d, *)
     * and msptiEnableAllDomains(sub) are called concurrently, the results are undefined.
     *
     * @param enable [in] New enable state for all callbacks in all domains. Zero disables all callbacks, non-zero
     ...
     * @return MSPTI_ERROR_NOT_INITIALIZED if MSPTI is not initialized
     * @return MSPTI_ERROR_WITHOUT_LD_PRELOAD if libmspti.so is not set in LD_PRELOAD environment variable
     * @return MSPTI_ERROR_INVALID_PARAMETER if @p subscriber is invalid.
     */
     ...
     * @param domain [in] The domain of the callback
     * @param cbid [in] The ID of the callback
     * @param name [out] Returns pointer to the name string on success, NULL otherwise
     *
     * @return MSPTI_SUCCESS on success
     * @return MSPTI_ERROR_INVALID_PARAMETER  if @p name is NULL, or if @p domain or @p cbid is invalid
     */
    msptiResult msptiGetCallbackName(msptiCallbackDomain domain, uint32_t cbid, const char **name);

    /**
     * @brief Get the current enabled/disabled state of a callback for a specific domain and function ID.
     *
     * Returns non-zero in @p enable if the callback for a domain and callback ID is enabled, and zero if not enabled.
     *
     * @note @b Thread-safety: a subscriber must serialize access to msptiGetCallbackState, msptiEnableCallback,
     * msptiEnableDomain, and msptiEnableAllDomains. For example, if msptiGetCallbackState(sub, d, c) and
     * msptiEnableCallback(sub, d, c) are called concurrently, the results are undefined.
     *
     * @param enable [out] Returns non-zero if callback enabled, zero if not enabled
     * @param subscriber [in] Handle to the initialize subscriber
     * @param domain [in] The domain of the callback
     * @param cbid [in] The ID of the callback
     *
     * @return MSPTI_SUCCESS on success
     * @return MSPTI_ERROR_NOT_INITIALIZED if MSPTI is not initialized
     * @return MSPTI_ERROR_INVALID_PARAMETER if @p enable is NULL, or if @p subscriber, @p domain or @p cbid is invalid
     */
     ...
     *
     * Returns in @p domainTable an array of size @p domainCount of all the available callback domains.
     *
     * @param domainCount [out] Returns number of callback domains
     * @param domainTable [out] Returns pointer to array of available callback domains
     *
     * @return MSPTI_SUCCESS on success
     * @return MSPTI_ERROR_INVALID_PARAMETER if @p domainCount or @p domainTable is NULL
     */
     ...
     *
     * If the provided buffer size is not sufficient to store all the enabled callbacks, we populate the buffer with
     * as much as we can, but return the true value of the number of enabled callbacks in @p enabledCallbacksCount.
     *
     * @param subscriber [in]  The subscriber handle
     * @param domain [in] The domain of the callbacks to get
     * @param buffer [out] The buffer to store the enabled callbacks. If NULL, the number of enabled callbacks is
     * returned in @p enabledCallbacksCount.
     * @param bufferSize [in] The size of the buffer. If NULL, only the number of enabled callbacks is returned in @p
     * enabledCallbacksCount. If NULL and @p buffer is not NULL, MSPTI_ERROR_INVALID_PARAMETER is returned.
     * @param enabledCallbacksCount [out] The number of enabled callbacks. If NULL, MSPTI_ERROR_INVALID_PARAMETER is
     * returned.
     *
     * @return MSPTI_SUCCESS on success
     * @return MSPTI_ERROR_NOT_INITIALIZED if MSPTI is not initialized
     * @return MSPTI_ERROR_INVALID_PARAMETER if @p subscriber, @p domain, @p buffer, @p bufferSize, or
     * @p enabledCallbacksCount is invalid
     */
    ...
    /**
     * @brief Check whether a MSPTI tracing session is still running.
     *
     * This API returns true if a MSPTI library is already loaded by another MSPTI user by using any MSPTI API.
     * This API returns false when MSPTI is finalized and there is no MSPTI currently loaded and active.
     * Can be used to determine if it is safe to unload your MSPTI based tool. Note that this API itself does not
     * load the MSPTI library, it merely checks if a MSPTI library is already loaded by another MSPTI user.
     *
     * @param isRunning [out] Returns whether the tracing session is still running.
     *
     * @return MSPTI_SUCCESS on success
     * @return MSPTI_ERROR_INVALID_PARAMETER if @p isRunning is NULL
     */
