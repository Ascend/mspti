...
本文档主要介绍msPTI工具的安装方式：**使用CANN包安装**和**源码编译安装**。

msPTI工具完整功能集成在CANN包中，且msPTI依赖CANN包，因此使用msPTI工具需要先[安装CANN包](#使用cann包安装)，若需要升级安装本工具代码仓中的最新功能，可以通过[源码编译安装](#可选源码编译安装)，在已安装CANN包的环境下覆盖安装msPTI包。
